...
bashssh user@app-server-2
Step 2: Install Nginx
bash# For RHEL/CentOS systems
sudo yum update -y
...
bashsudo vi /etc/nginx/conf.d/ssl.conf
...
bashecho "Welcome!" | sudo tee /usr/share/nginx/html/index.html
Step 6: Start and Enable Nginx
bash# Start nginx service
...
bash# Test HTTPS connection (headers only)
...
Troubleshooting
Nginx won't start
bash# Check nginx configuration syntax
sudo nginx -t

# Check nginx error logs
sudo tail -f /var/log/nginx/error.log
Certificate errors
bash# Verify certificate files exist and have correct permissions
ls -la /etc/nginx/ssl/

# Check certificate details
openssl x509 -in /etc/nginx/ssl/nautilus.crt -text -noout
Connection refused
bash# Verify nginx is listening on port 443
...
bash# If SELinux is enforcing, you may need to set proper context
