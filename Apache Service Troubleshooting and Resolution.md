...
bashsudo systemctl status httpd.service
...
Identified sendmail using port 3000
Stopped and disabled sendmail
Started and enabled Apache
Verified Apache running on port 3000
...
bash# On each app server (app-server-01, app-server-02, app-server-03):
...
Port Conflicts: Always check for port conflicts when a service fails to start with "Address already in use" errors
Useful Commands:
...
Service Dependencies: Consider service dependencies and port allocations when deploying multiple services
Systematic Approach: Follow a methodical troubleshooting process: check status → verify config → attempt start → analyze errors → identify root cause → apply fix → verify
